...
//! End-to-end crash-safety coverage: interrupt a *real* `patina apply`
//! process mid-materialize, then prove the next run converges.
//!
...
//! binary and kills it via the debug-only `PATINA_TEST_ABORT_AFTER_OP` seam
//! (the engine `std::process::exit`s after the k-th materialized operation,
//! before writing the COMMIT sentinel, an approximation of `kill -9`). It then
//! asserts the crash window (a flushed `<ts>.plan`, no `<ts>.COMMIT`) and
//! drives `recover_orphans` to confirm the filesystem returns to its pre-apply
//! state byte-for-byte. This exercises the engine's own recover-before-flush
//! wiring that the direct-staging tests cannot.
//!
//! Both cases converge to pre-apply because the interrupted apply never
//! committed: whatever ops completed are reversed from backups (overwrites) or
//! deleted (fresh creations).
...
/// A fixture whose repo declares two copy entries in a fixed order:
/// `~/.a` (declared first) overwrites a pre-existing unmanaged file, and
/// `~/.b` (declared second) is a fresh creation. Operations materialize in
/// declaration order, so `PATINA_TEST_ABORT_AFTER_OP=1` completes the overwrite
/// and leaves the fresh create un-started.
fn setup() -> Fixture {
    let fx = Fixture::new();
...
/// A `patina apply` killed after its first operation leaves a flushed plan and
/// no COMMIT; the next run's recovery reverses the completed overwrite from its
/// backup and leaves the un-started fresh create absent: the pre-apply state.
#[test]
fn kill_after_first_op_converges_to_pre_apply_on_recovery() {
    ...
    // Crash window: the plan is flushed but the apply never committed.
    assert_eq!(
...
/// A `patina apply` killed after all operations but before the COMMIT is still
/// an uncommitted orphan: recovery reverses every completed op back to the
/// pre-apply state rather than treating the run as successful.
#[test]
fn kill_after_all_ops_before_commit_converges_to_pre_apply_on_recovery() {
    ...
    // Intermediate state before recovery: both targets were materialized.
    assert_eq!(

//! Integration tests for `watch_foreground_cli`.
#![cfg_attr(
    unix,
    expect(
        clippy::expect_used,
        clippy::indexing_slicing,
        reason = "Integration tests use .expect() for fixtures and a bounded read-buffer slice outside #[cfg(test)] modules; allow-*-in-tests does not cover integration-crate helpers."
    )
)]

...
mod tests {
    ...
    fn validate_rejects_a_path_under_an_injected_system_dir() {
        // The env-derived denylist rule, tested with an injected directory so
        // it runs on any platform (the process env is untouched).
    ...
    fn classify_falls_back_to_the_ledger_when_the_list_is_withheld() {
        // Unmanaged is undetectable here: without the live list there is
        // nothing to notice an unrecorded-but-present exclusion
        // against.
    ...
    fn is_drive_root_guards_input_without_a_drive_and_colon() {
        // The early-return guard: a string too short to carry a drive letter
        // and colon is not a drive root. `validate_exclusion_path` only
        // ever reaches `is_drive_root` with a drive-absolute path, so
        // this guard is exercised directly.

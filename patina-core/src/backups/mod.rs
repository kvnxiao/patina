...
//! the original. After an apply flushes the plan and before its first write,
//! it calls [`backup_before_overwrite`] for every target it will overwrite or
//! remove, including one whose regular file becomes a symlink, outermost first
//! and skipping a target inside a target already backed up. That call copies
//! the original bytes to `<state>/patina/backups/<ts>/<mirrored-target-path>`,
//! staging them in a `.partial.<pid>` sibling that it renames into place. The
//! mirrored path is the inverse map crash recovery reads, so both

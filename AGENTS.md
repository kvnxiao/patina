...
- **Rollback fidelity.** After `patina rollback`, the filesystem matches pre-apply state in content and entry kind (file / symlink / directory). Mode and timestamp bits are excluded, as is an edit made after the apply to a target the apply left unchanged. Before rollback replaces or deletes any other entry that changed since the apply, it keeps a copy under `<state>/recovered/`.
...
- **Journal timestamps have one-second resolution.** An apply that starts within the second of the latest `<ts>.COMMIT` shares that file, collapsing the older record. `patina remove` and `patina promote` stamp their commit with a later second instead. The remote cache's reachability sweep depends on those records.

...
- **Recovery deletes a file created at a `Create` target after a crash.** Recovery removes whatever is at a target the interrupted apply planned as `Create`. A file the user creates at that path between the crash and the next command that recovers is deleted with it.

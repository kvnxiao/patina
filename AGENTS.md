...
- **CI script author.** Runs `patina apply` in a non-interactive shell to preview a deployment; expects plan output and no writes to the repository or to any target. Planning a remote-backed entry against a cold cache still fills the per-machine remote cache; the plan is computed from that cache.
...
- **No panics.** Correctness claims require passing tests.
...
- **The Held-path reap window is documented, not mitigated.** `patina remove` / `promote` write a new journal record for one target under a caller-held lock without prompting, and plan that re-apply without `Remove` operations. If execution fails after `remove` replaces its target and edits the manifest, and before COMMIT, the prior record remains authoritative, and the next apply plans a `Remove` for the replaced file. The diff shows that removal and the apply backs up the file first, but `patina rollback` does not restore a reaped target: the reaping apply's record omits it, so its bytes remain only in that apply's backup cycle.
- **An `Update` target deleted between planning and execution keeps Patina's bytes after a crash.** The backup pass backs up nothing for a target that is absent when it runs, so recovery reads the missing backup as an unstarted `Update` and leaves the written file in place instead of deleting it.
- **A user edit during `execute` is overwritten.** The apply backs up every target in one pass before its first write. An edit to a target after that pass and before the target's own write is overwritten, and neither the backup nor recovery keeps it.
- **A partially written `<ts>.COMMIT` counts as committed.** Recovery treats any `COMMIT` file as the end of its apply, so a write of the sentinel that fails partway leaves that apply neither reversed nor recovered, and status skips the torn record.
- **The interior-symlink gate checks only directories above a tree's current leaves.** Classification and the executors return `InteriorSymlink` for a foreign symlink at a directory between a tree root and a leaf the current source walk yields. A current tree root replaced by a link is handled separately: the reap skips every recorded target under it, the backup pass backs up nothing beneath a target it backed up, and recovery reverts an operation beneath a link stashed in the backup cycle as that link. Other paths follow links. The orphan reap removes a recorded target through a link planted above it after the apply, whether the target lies outside every current tree or inside a current tree root under a directory above no current leaf: with `~/app/sub` replaced by a link to `~/elsewhere`, dropping `sub/z.conf` from the source deletes `~/elsewhere/z.conf`. A `[[file]]` target inside a copy-mode `[[directory]]` whose root was replaced by a directory link is planned at the link's resolved path and written through the link, and the next apply reaps that file. The diff previews that write at the resolved path and each removal, and the apply backs up the entry the link reaches before it writes or removes it. `patina rollback` and crash recovery can also touch the destination of a link planted above a recorded target. Extending the gate to the revert paths is a follow-up.

...
- **The Held-path reap window is documented, not mitigated.** `patina remove` / `promote` write a new journal record for one target under a caller-held lock without prompting. If execution fails after the promotion write and before COMMIT, the prior record remains authoritative and the next apply can reap the promoted file. The reap backs up its bytes before removal, so `patina rollback` recovers them.

...
- **The Held-path reap window is documented, not mitigated.** `patina remove` / `promote` write a new journal record for one target under a caller-held lock without prompting, and plan that re-apply without `Remove` operations. If execution fails after `remove` replaces its target and edits the manifest, and before COMMIT, the prior record remains authoritative, and the next apply plans a `Remove` for the replaced file. The diff shows that removal and the reap backs up the file first, but `patina rollback` does not restore a reaped target: the reaping apply's record omits it, so its bytes remain only in that apply's backup cycle.
- **An `Update` target deleted between planning and execution keeps Patina's bytes after a crash.** The executor backs up nothing for a target that is absent when it runs, so recovery reads the missing backup as an unstarted `Update` and leaves the written file in place instead of deleting it.

...
| `remove` | Unmanage a target (replace it with a plain file holding the last-applied content); `--purge` deletes it outright. Refuses a single leaf of a tree-mode `[[directory]]` entry. |
| `promote` | Copy a drifted copy-mode target's current bytes back into its repo source, then re-apply. Refuses targets deployed from remotes. |
...
accept `--yes` to skip the prompt. `apply` and `status` accept repeatable
`-v key=value` overrides. Pass the same overrides to both commands when a
`when` expression reads an overridden variable.
